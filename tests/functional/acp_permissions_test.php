...
class phpbb_functional_acp_permissions_test extends phpbb_functional_test_case
{
	...
	*/
	public function test_change_permission($description, $permission_type, $permission, $route, $object_name, $object_id)
	{
		// Get the form
		$crawler = self::request('GET', "app.php/admin/permissions/$route?${object_name}[0]=$object_id&type=$permission_type&sid={$this->sid}");
		$this->assertContains($this->lang('ACL_SET'), $crawler->filter('h1')->eq(1)->text());
