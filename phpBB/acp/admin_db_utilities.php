...
*
*     $Id: admin_db_utilities.php,v 1.2 2011/03/05 10:29:55 orynider Exp $
*
****************************************************************************/
	...
	if ($row = $db->sql_fetchrow($result))
	{
		...
		do
		{
			...
			for ($j = 0; $j < $num_fields; $j++)
			{
				...
				elseif (!empty($row[$field_names[$j]]))
				{
	...
	switch($perform)
	{
		...
		case 'restore':
			...
			else
			{
				...
				//
				$backup_file_name = (!empty($_POST['backup_file']['name'])) ? $_POST['backup_file']['name'] : "";
				$backup_file_tmpname = ($_POST['backup_file']['tmp_name'] != "none") ? $_POST['backup_file']['tmp_name'] : "";
				$backup_file_type = (!empty($_POST['backup_file']['type'])) ? $_POST['backup_file']['type'] : "";
