 ...
 *
 *   $Id: admin_forums.php,v 1.1 2010/10/10 15:05:22 orynider Exp $
 *
 ***************************************************************************/
...
$row_color = ( !($mode) ) ? $theme['td_color1'] : $theme['td_color2'];
$row_class = ( !($mode) ) ? $theme['td_class1'] : $theme['td_class2'];
// ------------------
	...
	switch($mode)
	{
		...
		case 'createforum':
			...
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, forum_parent, forum_desc, forum_order, forum_status, prune_enable" . $field_sql . ")
				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $_POST['forumname']) . "', " . $new_cat . ', ' . $new_parent . ", '" . str_replace("\'", "''", $_POST['forumdesc']) . "', $next_order, " . intval($_POST['forumstatus']) . ", " . request_var('prune_enable', 0) . $value_sql . ")";
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, "Couldn't insert row in forums table", "", __LINE__, __FILE__, $sql);
			}

			if(is_post('prune_enable'))
			{
		...
		case 'modforum':
			...
			// End Simple Subforums MOD		
			$prune_enable = request_var('prune_enable', 0);
			// Modify a forum in the DB
			if(is_post('prune_enable', 0))
			{
				if($prune_enable !== 1)
				{
					$prune_enable = 0;
			...
			$forum_data_sql['forum_name'] = utf8_normalize_nfc($_POST['forumname']);			
			$forum_data_sql['cat_id'] = $new_cat;
			$forum_data_sql['forum_parent'] = $new_parent;
			$forum_data_sql['forum_desc'] = utf8_normalize_nfc($_POST['forumdesc']);
			$forum_data_sql['forum_status'] = intval($_POST['forumstatus']);
			$forum_data_sql['prune_enable'] = intval($prune_enable);
		...
		case 'movedelforum':
			...
			$delete_old = request_var('delete_old', '');
