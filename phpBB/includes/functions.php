...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum', $send_headers = true)
{
	...
	*/
	...
	$template->assign_vars(array(
		...
		'T_JQUERY_LINK'			=> !empty($config['allow_cdn']) && !empty($config['load_jquery_url']) ? $config['load_jquery_url'] : "{$web_path}assets" . (($config['enable_mod_rewrite']) ? '.' . $config['assets_version'] . '/javascript/jquery.min.js' : 'javascript/jquery.min.js?assets_version=' . $config['assets_version']),
