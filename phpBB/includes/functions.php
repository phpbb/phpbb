...
function confirm_box($check, $title = '', $hidden = '', $html_body = 'confirm_body.html', $u_action = '')
{
	...
	/** @var \phpbb\acp\helper\functions $acp_functions */
	$acp_functions = $phpbb_container->get('acp.functions');
		...
		switch ($result['status'])
		{
			case LOGIN_ERROR_PASSWORD_CONVERT:
				$err = sprintf(
					...
					($config['email_enable']) ? '<a href="' . $controller_helper->route('ucp_account', ['mode' => 'forgot_password']) . '">' : '',
...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum', $send_headers = true)
{
	...
	*/
	...
	$template->assign_vars(array(
		...
		'U_PRIVATEMSGS'			=> $controller_helper->route('ucp_pm_view', ['folder' => 'inbox']),
		'U_RETURN_INBOX'		=> $controller_helper->route('ucp_pm_view', ['folder' => 'inbox']),
		...
		'U_MODCP'				=> $controller_helper->route('mcp_index', [], true, $user->session_id),
		...
		'S_LOGIN_ACTION'		=> !defined('ADMIN_START') ? $controller_helper->route('ucp_account', ['mode' => 'login']) : $controller_helper->route('acp_index', [], true, $user->session_id),
