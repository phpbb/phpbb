			...
			case 'add':
				...
				if ($update)
				{
					...
					$submit_ary = array(
						...
						'ban'				=> isset($_REQUEST['group_ban']) ? 1 : 0,
						'warn'				=> isset($_REQUEST['group_warn']) ? 1 : 0,
				...
				$template->assign_vars(array(
					...
					'GROUP_BAN'				=> (!empty($group_row['group_ban'])) ? ' checked="checked"' : '',
					'GROUP_WARN'			=> (!empty($group_row['group_warn'])) ? ' checked="checked"' : '',
					'USER_GROUPS'			=> ($group_name == 'GUESTS' || $group_name == 'BOTS') ? false : true,
