		...
		switch ($mode)
		{
			...
			case 'avatar':
				...
				$template->assign_vars(array(
					...
					'L_AVATAR_EXPLAIN'	=> $user->lang('AVATAR_EXPLAIN', $config['avatar_max_width'], $config['avatar_max_height'], $config['avatar_filesize'] / 1024),
			...
			case 'sig':
				...
				$template->assign_vars(array(
					...
					'BBCODE_STATUS'			=> ($config['allow_sig_bbcode']) ? $user->lang('BBCODE_IS_ON', '<a href="' . append_sid("{$phpbb_root_path}faq.$phpEx", 'mode=bbcode') . '">', '</a>') : $user->lang('BBCODE_IS_OFF', '<a href="' . append_sid("{$phpbb_root_path}faq.$phpEx", 'mode=bbcode') . '">', '</a>'),
