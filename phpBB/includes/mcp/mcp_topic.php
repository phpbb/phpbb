	...
	*/
	...
	foreach ($rowset as $i => $row)
	{
		...
		$post_row = array(
			...
			'MESSAGE'		=> $message,
			'POST_ID'		=> $row['post_id'],
			'RETURN_TOPIC'	=> $user->lang('RETURN_TOPIC', '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", 't=' . $topic_id) . '">', '</a>'),
	...
	$template->assign_vars(array(
		...
		'TO_TOPIC_INFO'		=> ($to_topic_id) ? $user->lang('YOU_SELECTED_TOPIC', $to_topic_id, '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $to_topic_info['forum_id'] . '&amp;t=' . $to_topic_id) . '">' . $to_topic_info['topic_title'] . '</a>') : '',
		...
		'RETURN_TOPIC'		=> $user->lang('RETURN_TOPIC', '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f={$topic_info['forum_id']}&amp;t={$topic_info['topic_id']}&amp;start=$start") . '">', '</a>'),
		'RETURN_FORUM'		=> $user->lang('RETURN_FORUM', '<a href="' . append_sid("{$phpbb_root_path}viewforum.$phpEx", "f={$topic_info['forum_id']}&amp;start=$start") . '">', '</a>'),
...
//					continue;
		...
		$return_link = $user->lang('RETURN_TOPIC', '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']) . '">', '</a>') . '<br /><br />' . $user->lang('RETURN_NEW_TOPIC', '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
...
function merge_posts($topic_id, $to_topic_id)
{
	...
	if (confirm_box(true))
	{
		...
		if ($row)
		{
			$return_link .= $user->lang('RETURN_TOPIC', '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . '&amp;t=' . $topic_id) . '">', '</a>');
		}
		else
		{
		...
		$return_link .= (($return_link) ? '<br /><br />' : '') . $user->lang('RETURN_NEW_TOPIC', '<a href="' . append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
		$redirect = $request->variable('redirect', "{$phpbb_root_path}viewtopic.$phpEx?f=$to_forum_id&amp;t=$to_topic_id");
		$redirect = reapply_sid($redirect);
