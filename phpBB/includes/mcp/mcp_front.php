			...
			if ($total)
			{
				...
				while ($row = $db->sql_fetchrow($result))
				{
					$template->assign_block_vars('unapproved', array(
						...
						'ATTACH_ICON_IMG'	=> ($auth->acl_get('u_download') && $auth->acl_get('f_download', $row['forum_id']) && $row['post_attachment']) ? true : false,
					));
				}
				$db->sql_freeresult($result);
			}
		...
		if ($total)
		{
			...
			foreach ($pm_list as $message_id)
			{
				...
				$template->assign_block_vars('pm_report', array(
					...
					'ATTACH_ICON_IMG'		=> ($auth->acl_get('u_download') && $row['message_attachment']) ? true : false,
				));
			}
		}
