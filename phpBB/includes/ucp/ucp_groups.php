		...
		switch ($mode)
		{
			case 'membership':
				...
				if ($submit || isset($_POST['change_default']))
				{
					...
					{
						$row['group_name'] = phpbb_get_groupname_string($row['group_name'], $user);
			...
			case 'manage':
				...
				if ($group_id)
				{
					...
					$template->assign_vars(array(
						'GROUP_NAME'			=> phpbb_get_groupname_string($group_name, $user),
				...
				switch ($action)
				{
					...
					case 'default':
						...
						$group_row['group_name'] = phpbb_get_groupname_string($group_row['group_name'], $user);
					...
					default:
						...
						{
							$template->assign_block_vars('leader', array(
								'GROUP_NAME'	=> phpbb_get_groupname_string($value['group_name'], $user),
