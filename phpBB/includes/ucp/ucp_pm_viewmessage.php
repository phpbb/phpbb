...
function view_message($id, $mode, $folder_id, $msg_id, $folder, $message_row)
{
	...
	$msg_data = array(
		...
		'U_POST_REPLY_PM'	=> (($auth->acl_get('u_sendpm') || $auth->acl_get('u_pm_reply')) && $author_id != ANONYMOUS) ? "$url&amp;mode=compose&amp;action=reply&amp;f=$folder_id&amp;p=" . $message_row['msg_id'] : '',
		'U_POST_REPLY_ALL'	=> (($auth->acl_get('u_sendpm') || $auth->acl_get('u_pm_reply')) && $author_id != ANONYMOUS) ? "$url&amp;mode=compose&amp;action=reply&amp;f=$folder_id&amp;reply_to_all=1&amp;p=" . $message_row['msg_id'] : '',
