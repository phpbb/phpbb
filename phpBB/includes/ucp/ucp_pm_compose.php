...
function compose_pm($id, $mode, $action, $user_folders = array())
{
	...
	if ($action != 'edit')
	{
		...
		if ($config['allow_mass_pm'] && $auth->acl_get('u_masspm_group'))
		{
			...
			{
				$group_options .= '<option' . (($row['group_type'] == GROUP_SPECIAL) ? ' class="sep"' : '') . ' value="' . $row['group_id'] . '">' . phpbb_get_groupname_string($row['group_name'], $user) . '</option>';
			}
		...
		foreach ($_types as $type)
		{
			if (isset($result[$type]) && $result[$type])
			{
				while ($row = $db->sql_fetchrow($result[$type]))
				{
					if ($type == 'g')
					{
						$row['name'] = phpbb_get_groupname_string($row['group_name'], $user);
					}
