...
class phpbb_style_resource_locator implements phpbb_template_locator
{
	...
	*/
	public function parse_resource_path($path)
	{
		$components = array(
			'full' => $path,
			'protocol' => false,
		...
		$path = $paths[0];
		$components['hash'] = isset($paths[1]) ? $paths[1] : '';

		// Get parameters
		$paths = explode('?', $path, 2);
		$filename = $paths[0];
		$components['params'] = isset($paths[1]) ? $paths[1] : '';

		// Get protocol
		$paths = explode('://', $filename, 2);
		if (count($paths) == 2)
		{
			$components['protocol'] = $paths[0];
			$components['filename'] = $paths[1];
		}
		else
		{
	...
	* @param bool $urlencode If true, path components will be encoded with urlencode()
	* @return string URL
	*/
	public function join_resource_path($components, $urlencode = false)
	{
		if ($urlencode)
		{
			$paths = explode('/', $components['filename']);
			foreach ($paths as &$dir)
			{
				$dir = urlencode($dir);
			}
			$components['filename'] = implode('/', $paths);
		}

		$filename = ($components['protocol'] === false ? '' : $components['protocol'] . '://') . $components['filename'];
		if (strlen($components['params']))
		{
			$filename .= '?' . $components['params'];
		}
		if (strlen($components['hash']))
		{
			$filename .= '#' . $components['hash'];
		}
