...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		else if ($row['forum_type'] != FORUM_CAT)
		{
			...
			if ($row['forum_last_post_time'] > $forum_rows[$parent_id]['forum_last_post_time'] || $row['forum_last_post_time_real'] > $forum_rows[$parent_id]['forum_last_post_time'])
			{
				// moderators should see the real latest post information to account for unapproved posts
				// to save lots of if/else work later, just overwrite the currently used variables with the
				// "real" information if needed
				if ($auth->acl_get('m_approve', $forum_id) && $row['forum_last_post_time_real'] > $row['forum_last_post_time'])
				{
					$row['forum_last_post_id'] = $row['forum_last_post_id_real'];
					$row['forum_last_post_subject'] = $row['forum_last_post_subject_real'];
					$row['forum_last_post_time'] = $row['forum_last_post_time_real'];
					$row['forum_last_poster_id'] = $row['forum_last_poster_id_real'];
					$row['forum_last_poster_name'] = $row['forum_last_poster_name_real'];
					$row['forum_last_poster_colour'] = $row['forum_last_poster_colour_real'];
					$row['forum_id_last_post'] = $forum_id;
				}
