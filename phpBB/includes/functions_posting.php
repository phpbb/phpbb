...
function load_drafts($topic_id = 0, $forum_id = 0, $id = 0, $pm_action = '', $msg_id = 0)
{
	...
	global $phpbb_root_path, $phpbb_dispatcher, $phpbb_container, $phpEx;
	...
	*/
	...
	foreach ($draft_rows as $draft)
	{
		...
		else
		{
			...
			$params = array('d' => $draft['draft_id']);
			if ($pm_action)
			{
				$params['action'] = $pm_action;
			}
			if ($msg_id)
			{
				$params['p'] = $msg_id;
			}
			$insert_url = $controller_helper->route('phpbb_privatemessage_compose', $params);
		}
