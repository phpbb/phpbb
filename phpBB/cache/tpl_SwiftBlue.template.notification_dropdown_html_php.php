...
?>
				<li class="<?php if ($notifications_item['UNREAD']) {  ?> bg2<?php } ?><?php if ($notifications_item['STYLING']) {  ?> <?php echo isset($notifications_item['STYLING']) ? $notifications_item['STYLING'] : ''; ?><?php } ?><?php if (! $notifications_item['URL']) {  ?> no-url<?php } ?>">
					...
					<?php if ($notifications_item['UNREAD']) {  ?>
						<a href="<?php echo isset($notifications_item['U_MARK_READ']) ? $notifications_item['U_MARK_READ'] : ''; ?>" class="mark_read icon-mark" data-ajax="notification.mark_read" title="<?php echo isset($this->vars['L_MARK_READ']) ? $this->vars['L_MARK_READ'] : $this->lang('L_MARK_READ'); ?>">
							 <i class="icon fa-check-circle icon-xl fa-fw" aria-hidden="true"></i><span class="sr-only"><?php echo isset($this->vars['L_MARK_READ']) ? $this->vars['L_MARK_READ'] : $this->lang('L_MARK_READ'); ?></span>
						</a>
					<?php } ?>
				</li>
			<?php
