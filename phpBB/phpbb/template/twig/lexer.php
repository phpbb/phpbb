	...
	*/
	protected function fix_if_tokens($code)
	{
		...
		$callback = function($matches)
		{
			...
			// Replace .foo with loops.foo|default([])|length
			$inner = preg_replace('#(\s\(*!?)\.([a-zA-Z_0-9]+)([^a-zA-Z_0-9\.])#', '$1loops.$2|default([])|length$3', $inner);

			// Replace .foo.bar with foo.bar|default([])|length
			$inner = preg_replace('#(\s\(*!?)\.([a-zA-Z_0-9\.]+)([^a-zA-Z_0-9\.])#', '$1$2|default([])|length$3', $inner);
