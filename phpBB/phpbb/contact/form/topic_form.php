...
class topic_form extends form
{
	 ...
	 */
	protected function get_topic_row(int $topic_id)
	{
	...
	public function bind(): void
	{
		...
		$this->topic_id = $this->request->variable('topic_id', 0);
		$this->recipient_address = $this->request->variable('email', '');
		$this->recipient_name = $this->request->variable('name', '', true);
		$this->recipient_lang = $this->request->variable('lang', $this->config['default_lang']);
	...
	public function submit(messenger $messenger): void
	{
		...
		{
			$this->errors[] = $this->language->lang('EMPTY_ADDRESS_EMAIL');
		}

		if (!$this->recipient_name)
		{
			$this->errors[] = $this->language->lang('EMPTY_NAME_EMAIL');
		}
	...
	{
		return sprintf($this->language->lang('RETURN_TOPIC'),  '<a href="' . append_sid($this->phpbb_root_path . 'viewtopic.' . $this->phpEx, 'f=' . $this->topic_row['forum_id'] . '&amp;t=' . $this->topic_id) . '">', '</a>');
	}
	...
	public function render(): void
	{
		...
		$this->template->assign_vars(array(
			...
			'L_EMAIL_BODY_EXPLAIN'	=> $this->language->lang('EMAIL_TOPIC_EXPLAIN'),
			'S_POST_ACTION'			=> $controller_helper->route('phpbb_contact_topic', ['topic_id' => $this->topic_id]),
		));
	}
}
