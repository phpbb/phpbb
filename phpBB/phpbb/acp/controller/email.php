		...
		if ($submit)
		{
			...
			if (empty($errors))
			{
				...
				if ($generate_log_entry)
				{
					if (!empty($usernames))
					{
						$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_MASS_EMAIL', false, [implode(', ', utf8_normalize_nfc($usernames))]);
					}
					else
					{
						if ($group_id)
						...
						$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_MASS_EMAIL', false, [$group_name]);
					}
				}
