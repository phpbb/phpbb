...
class permissions
{
	...
	public function main($mode)
	{
		...
		switch ($mode)
		{
			...
			case 'intro':
				...
				return $this->helper->render('acp_permissions.html', $this->language->lang('ACP_PERMISSIONS_INTRO'));

			// Trace has other vars
			case 'trace':
				$user_ids = $this->request->variable('u', 0);
				$forum_ids = $this->request->variable('f', 0);
				$permission = $this->request->variable('auth', '');

				if ($user_ids && isset($this->auth_admin->acl_options['id'][$permission]) && $this->auth->acl_get('a_viewauth'))
				{
					return $this->permission_trace($user_ids, $forum_ids, $permission);
				}

				return trigger_error($this->language->lang('NO_MODE') . $this->helper->adm_back_route('acp_permissions'), E_USER_ERROR);
		...
		switch ($mode)
		{
			...
			default:
				return trigger_error($this->language->lang('NO_MODE') . $this->helper->adm_back_route('acp_permissions'), E_USER_WARNING);
			break;
		}
		...
		if ($select_all_groups)
		{
			...
			$sql = 'SELECT group_id
				...
				WHERE group_type = ' . GROUP_SPECIAL . $sql_and;
		...
		if (!empty($username) && empty($user_ids))
		{
			...
			if (empty($user_ids))
			{
				return trigger_error($this->language->lang('SELECTED_USER_NOT_EXIST') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
			}
		}
		unset($username);
		...
		{
			return trigger_error($this->language->lang('WRONG_PERMISSION_TYPE') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
		}
			...
			switch ($action)
			{
				case 'delete':
					if (confirm_box(true))
					{
						...
						{
							return trigger_error($this->language->lang('NO_USER_GROUP_SELECTED') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
						}
					}
					else
					{
				...
				case 'apply_permissions':
					if (!$this->request->is_set_post('setting'))
					{
						...
						return trigger_error($this->language->lang('NO_AUTH_SETTING_FOUND') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
					}
		...
		foreach ($victims as $victim)
		{
			...
			$this->template->assign_vars([
				...
				'S_CAN_SELECT_USER'		=> (bool) $this->auth->acl_get('a_authusers'),
				'S_CAN_SELECT_GROUP'	=> (bool) $this->auth->acl_get('a_authgroups'),
			...
			return $this->helper->render('acp_permissions.html', $this->language->lang($l_mode));
		}
		...
		{
			return trigger_error($this->language->lang('ONLY_FORUM_DEFINED') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
		}
	...
	protected function check_existence($mode, $u_mode, array &$ids)
	{
		...
		if (empty($ids))
		{
			trigger_error($this->language->lang('SELECTED_' . strtoupper($mode) . '_NOT_EXIST') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
		}
	}
	...
	protected function set_permissions($mode, $u_mode, $permission_type, array &$user_ids, array &$group_ids)
	{
		...
		{
			return trigger_error('WRONG_PERMISSION_SETTING_FORMAT', E_USER_WARNING);
		}
		...
		if (!empty($inherit))
		{
			foreach ($inherit as $_ug_id => $forum_ids_ary)
			{
				...
				// Inherit forums?
				$forum_ids = array_merge($forum_ids, array_keys($forum_ids_ary));
			}
		}
		...
		$this->auth_admin->acl_set($ug_type, $forum_ids, $ug_id, $auth_settings, $assigned_role);
		...
		$this->log_action($mode, 'add', $permission_type, $ug_type, $ug_id, $forum_ids);
	...
	protected function check_assigned_role($role_id, array &$auth_settings)
	{
		...
		$sql = 'SELECT o.auth_option, r.auth_setting
			FROM ' . $this->tables['acl_options'] . ' o,
				' . $this->tables['acl_roles_data'] . ' r
			WHERE o.auth_option_id = r.auth_option_id
				AND r.role_id = ' . (int) $role_id;
		$result = $this->db->sql_query($sql);
		while ($row = $this->db->sql_fetchrow($result))
		{
			$test_settings[$row['auth_option']] = $row['auth_setting'];
		}
		$this->db->sql_freeresult($result);
		...
		foreach ($auth_settings as $option => $setting)
		{
			if ($setting == ACL_NO)
			{
				$test_settings[$option] = $setting;
			}
		}

		if (!empty(array_diff_assoc($auth_settings, $test_settings)))
		{
			return false;
		}
	...
	protected function log_action($mode, $action, $permission_type, $ug_type, $ug_id, $forum_ids)
	{
		...
		if ($ug_type === 'group')
		{
			$sql = 'SELECT group_name, group_type
				FROM ' . $this->tables['groups'] . '
				WHERE ' . $this->db->sql_in_set('group_id', array_map('intval', $ug_id));
			$result = $this->db->sql_query($sql);
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				$ug_list .= $ug_list !== '' ? ', ' : '';
				$ug_list .= $row['group_type'] == GROUP_SPECIAL ? '<span class="sep">' . $group_name . '</span>' : $group_name;
			}
			$this->db->sql_freeresult($result);
		}
		else
		{
			$sql = 'SELECT username
				...
				WHERE ' . $this->db->sql_in_set('user_id', array_map('intval', $ug_id));
			$result = $this->db->sql_query($sql);
			while ($row = $this->db->sql_fetchrow($result))
			{
				$ug_list .= $ug_list !== '' ? ', ' : '';
				$ug_list .= $row['username'];
			}
			$this->db->sql_freeresult($result);
		}
		...
		if ($forum_ids[0] == 0)
		{
			$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_ACL_' . strtoupper($action) . '_' . strtoupper($mode) . '_' . strtoupper($permission_type), false, [$ug_list]);
		}
		else
		{
			$forum_list = '';
			...
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				$forum_list .= $row['forum_name'];
			}
			$this->db->sql_freeresult($result);

			$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_ACL_' . strtoupper($action) . '_' . strtoupper($mode) . '_' . strtoupper($permission_type), false, [$forum_list, $ug_list]);
		}
	}
	...
	protected function permission_trace($user_id, $forum_id, $permission)
	{
		...
		if (!$userdata)
		{
			return trigger_error($this->language->lang('NO_USERS') . $this->helper->adm_back_route('acp_permissions'), E_USER_ERROR);
		}
		...
		if (!empty($groups))
		{
			...
			foreach ($groups as $id => $row)
			{
				...
				$this->template->assign_block_vars('trace', [
					'WHO'				=> $row['group_name'],
					'INFORMATION'		=> $this->language->lang($information),
		...
		if ($forum_id != 0 && isset($this->auth->acl_options['global'][$permission]))
		{
			...
			if ($auth_setting)
			{
				...
				$this->template->assign_block_vars('trace', [
					'WHO'				=> $this->language->lang('TRACE_GLOBAL_SETTING', $userdata['username']),
					'INFORMATION'		=> $this->language->lang($information, '<a href="' . $u_info . '">', '</a>'),
		...
		return $this->helper->render('permission_trace.html', $this->language->lang('TRACE_PERMISSION', $this->permissions->get_permission_lang($permission)));
	}
	...
	protected function copy_forum_permissions()
	{
		...
		if ($submit)
		{
			...
			if (confirm_box(true))
			{
				...
				{
					return trigger_error($this->language->lang('SELECTED_FORUM_NOT_EXIST') . $this->helper->adm_back_route('acp_permissions_forum_copy'), E_USER_WARNING);
				}
			}
			else
			{
				confirm_box(false, $this->language->lang('COPY_PERMISSIONS_CONFIRM'), build_hidden_fields([
					'submit'			=> $submit,
					'src_forum_id'		=> $src,
					'dest_forum_ids'	=> $dest,
				]));
		...
		return $this->helper->render('permission_forum_copy.html', $this->language->lang('ACP_FORUM_PERMISSIONS_COPY'));
	}
	...
	protected function retrieve_defined_user_groups($permission_scope, array $forum_ids, $permission_type)
	{
		...
		$sql_forum_id = $permission_scope === 'global'
			? 'AND a.forum_id = 0'
			: (!empty($forum_ids)
				? 'AND ' . $this->db->sql_in_set('a.forum_id', array_map('intval', $forum_ids))
				: 'AND a.forum_id <> 0');

		// Permission options are only able to be a permission set...
		// Therefore we will pre-fetch the possible options and also the possible roles
