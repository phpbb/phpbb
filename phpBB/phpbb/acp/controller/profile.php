...
class profile
{
	...
	public function main($id, $mode)
	{
		...
		$action = ($this->request->is_set_post('create')) ? 'create' : $this->request->variable('action', '');
		...
		switch ($action)
		{
			case 'delete':
				...
				if (confirm_box(true))
				{
					...
					$this->db->sql_query('DELETE FROM ' . PROFILE_FIELDS_TABLE . " WHERE field_id = $field_id");
					$this->db->sql_query('DELETE FROM ' . PROFILE_FIELDS_LANG_TABLE . " WHERE field_id = $field_id");
					$this->db->sql_query('DELETE FROM ' . PROFILE_LANG_TABLE . " WHERE field_id = $field_id");
					...
					trigger_error($this->language->lang('REMOVED_PROFILE_FIELD') . adm_back_link($this->u_action));
				}
				else
				{
			...
			case 'activate':
				...
				trigger_error($this->language->lang('PROFILE_FIELD_ACTIVATED') . adm_back_link($this->u_action));
			...
			case 'deactivate':
				...
				trigger_error($this->language->lang('PROFILE_FIELD_DEACTIVATED') . adm_back_link($this->u_action));
			...
			case 'edit':
				...
				$submit = ($this->request->is_set('next') || $this->request->is_set('prev')) ? true : false;
				$save = ($this->request->is_set('save')) ? true : false;
				...
				$cp->vars['field_ident']		= ($action == 'create' && $step == 1) ? utf8_clean_string($this->request->variable('field_ident', $field_row['field_ident'], true)) : $this->request->variable('field_ident', $field_row['field_ident']);
				...
				$cp->vars['field_contact_desc']	= $this->request->variable('field_contact_desc', $field_row['field_contact_desc'], true);
				$cp->vars['field_contact_url']	= $this->request->variable('field_contact_url', $field_row['field_contact_url'], true);
				...
				$cp->vars['field_no_view'] = $this->request->variable('field_no_view', (int) $field_row['field_no_view']);
				...
				switch ($step)
				{
					...
					case 1:
						$template_vars = [
							...
							'L_LANG_SPECIFIC'	=> sprintf($this->language->lang('LANG_SPECIFIC_OPTIONS'), $this->config['default_lang']),
					...
					case 2:
						...
						$options = $profile_field->get_options($this->lang_defs['iso'][$this->config['default_lang']], $cp->vars);
					...
					case 3:
						...
						foreach ($options as $lang_id => $lang_ary)
						{
							$this->template->assign_block_vars('options', [
								'LANGUAGE'		=> sprintf($this->language->lang[(($lang_id == $this->edit_lang_id) ? 'DEFAULT_' : '') . 'ISO_LANGUAGE'], $lang_ary['lang_iso'])]
							);
	...
	function build_language_options($cp, $field_type, $action = 'create')
	{
		...
		foreach ($options as $field => $field_type)
		{
			...
			if (isset($this->language->lang('CP_' . strtoupper($field) . '_EXPLAIN')))
			{
		...
		foreach ($languages as $lang_id => $lang_iso)
		{
			...
			foreach ($options as $field => $field_type)
			{
				$value = ($action == 'create') ? $this->request->variable('l_' . $field, [0 => ''], true) : $cp->vars['l_' . $field];
	...
	function save_profile_field($cp, $field_type, $action = 'create')
	{
		...
		if ($action == 'create')
		{
			...
			$sql = 'INSERT INTO ' . PROFILE_FIELDS_TABLE . ' ' . $this->db->sql_build_array('INSERT', $profile_fields);
			$this->db->sql_query($sql);
		...
		if ($action == 'create')
		{
			...
			/* @var $db_tools \phpbb\db\tools\tools_interface */
			$db_tools = $phpbb_container->get('dbal.tools');
			$this->db_tools->sql_column_add(PROFILE_FIELDS_DATA_TABLE, $field_ident, [$profile_field->get_database_column_type(), null]);
		}
		...
		if ($action == 'create')
		{
			...
			$profile_sql[] = 'INSERT INTO ' . PROFILE_LANG_TABLE . ' ' . $this->db->sql_build_array('INSERT', $sql_ary);
		}
		else
		{
		...
		if ($cp->vars['lang_options'])
		{
			...
			foreach ($cp->vars['lang_options'] as $option_id => $value)
			{
				...
				if ($action == 'create')
				{
					...
					$profile_sql[] = 'INSERT INTO ' . PROFILE_FIELDS_LANG_TABLE . ' ' . $this->db->sql_build_array('INSERT', $sql_ary);
				}
				else
				{
		...
		foreach ($profile_lang as $sql)
		{
			if ($action == 'create')
			{
				$profile_sql[] = 'INSERT INTO ' . PROFILE_LANG_TABLE . ' ' . $this->db->sql_build_array('INSERT', $sql);
			}
			else
			{
				$lang_id = $sql['lang_id'];
		...
		if (count($profile_lang_fields))
		{
			foreach ($profile_lang_fields as $sql)
			{
				if ($action == 'create')
				{
					$profile_sql[] = 'INSERT INTO ' . PROFILE_FIELDS_LANG_TABLE . ' ' . $this->db->sql_build_array('INSERT', $sql);
				}
				else
				{
					$lang_id = $sql['lang_id'];
		...
		if ($action == 'edit')
		{
			$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_PROFILE_FIELD_EDIT', false, [$cp->vars['field_ident'] . ':' . $cp->vars['lang_name']]);
			trigger_error($this->language->lang('CHANGED_PROFILE_FIELD') . adm_back_link($this->u_action));
		}
		else
		{
			$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_PROFILE_FIELD_CREATE', false, [substr($field_ident, 3) . ':' . $cp->vars['lang_name']]);
			trigger_error($this->language->lang('ADDED_PROFILE_FIELD') . adm_back_link($this->u_action));
		}
	}
