...
class profile
{
		...
		switch ($action)
		{
			...
			case 'activate':
				...
				if ($this->request->is_ajax())
				{
					...
					$json_response->send([
						'text'	=> $this->lang->lang('DEACTIVATE'),
					]);
				}
			...
			case 'edit':
				...
				if ($submit) // && $step == 1
				{
					...
					{
						$errors[] = $this->lang->lang('EMPTY_FIELD_IDENT');
					}

					if (!preg_match('/^[a-z_]+$/', $this->pf_manager->vars['field_ident']))
					{
						$errors[] = $this->lang->lang('INVALID_CHARS_FIELD_IDENT');
					}

					if (strlen($this->pf_manager->vars['field_ident']) > 17)
					{
						$errors[] = $this->lang->lang('INVALID_FIELD_IDENT_LEN');
					}

					if ($this->pf_manager->vars['lang_name'] == '')
					{
						$errors[] = $this->lang->lang('EMPTY_USER_FIELD_NAME');
					}
					...
					if ($action !== 'edit')
					{
						...
						if ($row)
						{
							$errors[] = $this->lang->lang('FIELD_IDENT_ALREADY_EXIST');
						}
					}
				}
				...
				switch ($step)
				{
					...
					case 1:
						$template_vars = [
							'S_STEP_ONE'		=> true,
							'S_FIELD_REQUIRED'	=> ($this->pf_manager->vars['field_required']) ? true : false,
							'S_FIELD_SHOW_NOVALUE'=> ($this->pf_manager->vars['field_show_novalue']) ? true : false,
							'S_SHOW_ON_REG'		=> ($this->pf_manager->vars['field_show_on_reg']) ? true : false,
							'S_SHOW_ON_PM'		=> ($this->pf_manager->vars['field_show_on_pm']) ? true : false,
							'S_SHOW_ON_VT'		=> ($this->pf_manager->vars['field_show_on_vt']) ? true : false,
							'S_SHOW_ON_MEMBERLIST'=> ($this->pf_manager->vars['field_show_on_ml']) ? true : false,
							'S_FIELD_HIDE'		=> ($this->pf_manager->vars['field_hide']) ? true : false,
							'S_SHOW_PROFILE'	=> ($this->pf_manager->vars['field_show_profile']) ? true : false,
							'S_FIELD_NO_VIEW'	=> ($this->pf_manager->vars['field_no_view']) ? true : false,
							...
							'L_LANG_SPECIFIC'	=> sprintf($this->lang->lang('LANG_SPECIFIC_OPTIONS'), $this->config['default_lang']),
					...
					case 2:
						$this->template->assign_vars([
							'S_STEP_TWO'	=> true,
							'L_NEXT_STEP'	=> count($this->lang_defs['iso']) === 1 ? $this->lang->lang('SAVE') : $this->lang->lang('PROFILE_LANG_OPTIONS'),
						]);
					...
					case 3:
						...
						foreach ($options as $lang_id => $lang_ary)
						{
							...
							$this->template->assign_block_vars('options', [
								'LANGUAGE'	=> $this->lang->lang($default . 'ISO_LANGUAGE', $lang_ary['lang_iso']),
							]);
	...
	function build_language_options($field_type, $action = 'create')
	{
		...
		foreach ($options as $field => $type)
		{
			...
			if ($this->lang->is_set('CP_' . strtoupper($field) . '_EXPLAIN'))
			{
				$lang_options[1]['fields'][$field]['EXPLAIN'] = $this->lang->lang('CP_' . strtoupper($field) . '_EXPLAIN');
			}
		}
		...
		foreach ($languages as $lang_id => $lang_iso)
		{
			...
			foreach ($options as $field => $type)
			{
				...
				if ($field == 'lang_options')
				{
					...
					switch ($type)
					{
						case 'two_options':
							$lang_options[$lang_id]['fields'][$field] = [
								'TITLE'		=> $this->lang->lang('CP_' . strtoupper($field)),
								'FIELD'		=> '
											<dd><input class="medium" name="l_' . $field . '[' . $lang_id . '][]" value="' . ((isset($value[$lang_id][0])) ? $value[$lang_id][0] : $var[0]) . '" /> ' . $this->lang->lang('FIRST_OPTION') . '</dd>
											<dd><input class="medium" name="l_' . $field . '[' . $lang_id . '][]" value="' . ((isset($value[$lang_id][1])) ? $value[$lang_id][1] : $var[1]) . '" /> ' . $this->lang->lang('SECOND_OPTION') . '</dd>',
							];
						break;
					...
					if ($this->lang->is_set('CP_' . strtoupper($field) . '_EXPLAIN'))
					{
						$lang_options[$lang_id]['fields'][$field]['EXPLAIN'] = $this->lang->lang('CP_' . strtoupper($field) . '_EXPLAIN');
					}
				}
				else
				{
					...
					$lang_options[$lang_id]['fields'][$field] = [
						'TITLE'		=> $this->lang->lang('CP_' . strtoupper($field)),
						'FIELD'		=> $type === 'string' ? '<dd><input class="medium" type="text" name="l_' . $field . '[' . $lang_id . ']" value="' . ((isset($value[$lang_id])) ? $value[$lang_id] : $var) . '" /></dd>' : '<dd><textarea name="l_' . $field . '[' . $lang_id . ']" rows="3" cols="80">' . (isset($value[$lang_id]) ? $value[$lang_id] : $var) . '</textarea></dd>',
					];
