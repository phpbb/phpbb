...
class main
{
	...
	public function main()
	{
				...
				switch ($action)
				{
					case 'online':
						if (!$this->auth->acl_get('a_board'))
						{
							...
							return trigger_error($this->language->lang('NO_AUTH_OPERATION') . $this->helper->adm_back_route('acp_index'), E_USER_WARNING);
						}
					...
					case 'purge_cache':
						...
						/**
						 * Remove old renderer from the text_formatter service.
						 * Since this operation is performed after the cache is purged,
						 * there is no "current" renderer and in effect all renderers will be purged.
						 */
						$this->tf_cache->tidy();
		...
		$this->template->assign_vars([
			...
			'S_VERSIONCHECK'	=> (bool) $this->auth->acl_get('a_board'),
			'S_ACTION_OPTIONS'	=> (bool) $this->auth->acl_get('a_board'),
		...
		if ($this->config['num_posts'] && class_exists($this->config['search_type']))
		{
			...
			if (!$search->index_created())
			{
				$this->template->assign_vars([
					...
					'L_NO_SEARCH_INDEX'			=> $this->language->lang('NO_SEARCH_INDEX', $search->get_name(), '<a href="' . $this->helper->route('acp_search_index') . '">', '</a>'),
				]);
			}
		}
