...
class board
{
	...
	public function main($mode)
	{
		...
		switch ($mode)
		{
			case 'board':
				$display_vars = [
					'vars'	=> [
						...
						'legend3'				=> 'WARNINGS',
						'warnings_expire_days'	=> ['lang' => 'WARNINGS_EXPIRE', 'validate' => 'int:0:9999', 'type' => 'number:0:9999', 'explain' => true, 'append' => ' ' . $this->language->lang('DAYS')],
			...
			case 'avatar':
				...
				$display_vars = [
					'vars'	=> [
						...
						'avatar_min'			=> ['lang' => 'MIN_AVATAR_SIZE', 'validate' => 'int:0', 'type' => 'dimension:0', 'explain' => true, 'append' => ' ' . $this->language->lang('PIXEL')],
						'avatar_max'			=> ['lang' => 'MAX_AVATAR_SIZE', 'validate' => 'int:0', 'type' => 'dimension:0', 'explain' => true, 'append' => ' ' . $this->language->lang('PIXEL')],
					],
				];
			...
			case 'post':
				$display_vars = [
					'vars'	=> [
						...
						'legend2'				=> 'POSTING',
						'bump_type'				=> false,
						'edit_time'				=> ['lang' => 'EDIT_TIME', 'validate' => 'int:0:99999', 'type' => 'number:0:99999', 'explain' => true, 'append' => ' ' . $this->language->lang('MINUTES')],
						...
						'max_post_img_width'	=> ['lang' => 'MAX_POST_IMG_WIDTH', 'validate' => 'int:0:9999', 'type' => 'number:0:9999', 'explain' => true, 'append' => ' ' . $this->language->lang('PIXEL')],
						'max_post_img_height'	=> ['lang' => 'MAX_POST_IMG_HEIGHT', 'validate' => 'int:0:9999', 'type' => 'number:0:9999', 'explain' => true, 'append' => ' ' . $this->language->lang('PIXEL')],
			...
			case 'signature':
				$display_vars = [
					'vars'	=> [
						...
						'max_sig_img_width'		=> ['lang' => 'MAX_SIG_IMG_WIDTH', 'validate' => 'int:0:9999', 'type' => 'number:0:9999', 'explain' => true, 'append' => ' ' . $this->language->lang('PIXEL')],
						'max_sig_img_height'	=> ['lang' => 'MAX_SIG_IMG_HEIGHT', 'validate' => 'int:0:9999', 'type' => 'number:0:9999', 'explain' => true, 'append' => ' ' . $this->language->lang('PIXEL')],
			...
			case 'load':
				$display_vars = [
					'vars'	=> [
						...
						'session_length'			=> ['lang' => 'SESSION_LENGTH', 'validate' => 'int:60:9999999999', 'type' => 'number:60:9999999999', 'explain' => true, 'append' => ' ' . $this->language->lang('SECONDS')],
						'active_sessions'			=> ['lang' => 'LIMIT_SESSIONS', 'validate' => 'int:0:9999', 'type' => 'number:0:9999', 'explain' => true],
						'load_online_time'			=> ['lang' => 'ONLINE_LENGTH', 'validate' => 'int:0:999', 'type' => 'number:0:999', 'explain' => true, 'append' => ' ' . $this->language->lang('MINUTES')],
						'read_notification_expire_days' => ['lang' => 'READ_NOTIFICATION_EXPIRE_DAYS', 'validate' => 'int:0', 'type' => 'number:0', 'explain' => true, 'append' => ' ' . $this->language->lang('DAYS')],
			...
			case 'security':
				$display_vars = [
					'vars'	=> [
						...
						'ip_login_limit_time'	=> ['lang' => 'IP_LOGIN_LIMIT_TIME', 'validate' => 'int:0:99999', 'type' => 'number:0:99999', 'explain' => true, 'append' => ' ' . $this->language->lang('SECONDS')],
			...
			case 'email':
				$display_vars = [
					'vars'	=> [
						...
						'email_max_chunk_size'	=> ['lang' => 'EMAIL_MAX_CHUNK_SIZE', 'validate' => 'int:1:99999', 'type' => 'number:1:99999', 'explain' => true],
			...
			default:
				return trigger_error($this->language->lang('NO_MODE') . $this->helper->adm_back_route('acp_index'), E_USER_WARNING);
			break;
		}
		...
		{
			$this->language->add_lang($display_vars['lang']);
		}
			...
			if ($submit && (($cfg_array['auth_method'] != $this->new_config['auth_method']) || $updated_auth_settings))
			{
				...
				if (array_key_exists('auth.provider.' . $method, $this->auth_providers))
				{
					...
					if ($error = $provider->init())
					{
						...
						return trigger_error($error . $this->helper->adm_back_route($u_mode), E_USER_ERROR);
					}
					$this->config->set('auth_method', basename($cfg_array['auth_method']));
				}
				else
				{
					return trigger_error($this->language->lang('NO_AUTH_PLUGIN') . $this->helper->adm_back_route($u_mode), E_USER_ERROR);
				}
			}
		}
		...
		if ($mode === 'email' && $this->request->is_set_post('send_test_email'))
		{
			...
			else
			{
				...
				return trigger_error($this->language->lang('EMAIL_DISABLED') . $this->helper->adm_back_route($u_mode), E_USER_WARNING);
			}
		}
		...
		if ($submit)
		{
			...
			{
				$message .= '<br /><br />' . $this->language->lang('ACC_ACTIVATION_WARNING');
		...
		return $this->helper->render('acp_board.html', $this->language->lang($l_mode));
	}
	...
	public function select_auth_method($selected_method, $key = '')
	{
		...
		foreach ($this->auth_providers as $key => $value)
		{
	...
	public function mail_auth_select($selected_method, $key = '')
	{
		...
		{
			$s_smtp_auth_options .= '<option value="' . $method . '"' . ($selected_method == $method ? ' selected="selected"' : '') . '>' . $this->language->lang('SMTP_' . str_replace('-', '_', $method)) . '</option>';
		}
	...
	{
		return '<option value="1"' . ($value == 1 ? ' selected="selected"' : '') . '>' . $this->language->lang('DELETE_OLDEST_MESSAGES') . '</option><option value="2"' . ($value == 2 ? ' selected="selected"' : '') . '>' . $this->language->lang('HOLD_NEW_MESSAGES_SHORT') . '</option>';
	}
	...
	{
		return h_radio('config[ip_check]', [4 => 'ALL', 3 => 'CLASS_C', 2 => 'CLASS_B', 0 => 'NO_IP_VALIDATION'], $value, $key);
	}
	...
	public function select_acc_activation($selected_value, $value)
	{
		...
		foreach ($act_array as $key => $data)
		{
			list($available, $value) = $data;
			$selected = $selected_value == $value ? ' selected="selected"' : '';
			$class = !$available ? ' class="disabled-option"' : '';
			$act_options .= '<option value="' . $value . '"' . $selected . $class . '>' . $this->language->lang($key) . '</option>';
		}
	...
	{
		return '<input id="' . $key . '" type="number" min="1" max="999" name="config[min_name_chars]" value="' . $value . '" /> ' . $this->language->lang('MIN_CHARS') . '&nbsp;&nbsp;<input type="number" min="8" max="180" name="config[max_name_chars]" value="' . $this->new_config['max_name_chars'] . '" /> ' . $this->language->lang('MAX_CHARS');
	}
	...
	{
		$user_char_options = '';
		$user_char_array = ['USERNAME_CHARS_ANY', 'USERNAME_ALPHA_ONLY', 'USERNAME_ALPHA_SPACERS', 'USERNAME_LETTER_NUM', 'USERNAME_LETTER_NUM_SPACERS', 'USERNAME_ASCII'];

		foreach ($user_char_array as $user_type)
		{
			$selected = $selected_value == $user_type ? ' selected="selected"' : '';
			$user_char_options .= '<option value="' . $user_type . '"' . $selected . '>' . $this->language->lang($user_type) . '</option>';
		}
	...
	{
		return '<input id="' . $key . '" type="number" min="1" max="999" name="config[min_pass_chars]" value="' . $value . '" /> ' . $this->language->lang('MIN_CHARS');
	}
	...
	{
		$pass_char_options = '';
		$pass_type_array = ['PASS_TYPE_ANY', 'PASS_TYPE_CASE', 'PASS_TYPE_ALPHA', 'PASS_TYPE_SYMBOL'];

		foreach ($pass_type_array as $pass_type)
		{
			$selected = $selected_value == $pass_type ? ' selected="selected"' : '';
			$pass_char_options .= '<option value="' . $pass_type . '"' . $selected . '>' . $this->language->lang($pass_type) . '</option>';
		}
	...
	public function bump_interval($value, $key)
	{
		...
		foreach ($types as $type => $lang)
		{
			...
			$s_bump_type .= '<option value="' . $type . '"' . $selected . '>' . $this->language->lang($lang) . '</option>';
		}
	...
	public function quick_reply($value, $key)
	{
		...
		return h_radio('config[allow_quick_reply]', $radio_ary, $value) .
			'<br /><br /><input class="button2" type="submit" id="' . $key . '_enable" name="' . $key . '_enable" value="' . $this->language->lang('ALLOW_QUICK_REPLY_BUTTON') . '" />';
	}
	...
	public function dateformat_select($value, $key)
	{
		...
		if (!isset($this->language->get_lang_array()['dateformats'][$value]))
		{
	...
	public function enable_mod_rewrite($value, $key)
	{
		...
		return h_radio($field_name, [1 => 'YES', 0 => 'NO'], $value) .
			($message !== false ? '<br /><span>' . $this->language->lang($message) . '</span>' : '');
	}
	...
	{
		return '<input class="button2" type="submit" id="' . $key . '" name="' . $key . '" value="' . $this->language->lang('SEND_TEST_EMAIL') . '" />';
	}
}
