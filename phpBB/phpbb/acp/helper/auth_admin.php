...
class auth_admin extends \phpbb\auth\auth
{
		...
		if (count($forum_ids) === 1 || ($local && count($ug_names_ary) > 1))
		{
			...
			foreach ($hold_ary as $forum_id => $forum_array)
			{
				...
				foreach ($content_array as $ug_id => $ug_array)
				{
					...
					{
						$role_desc = $this->language->is_set($role_row['role_description']) ? $this->language->lang($role_row['role_description']) : nl2br($role_row['role_description']);
						$role_name = $this->language->lang($role_row['role_name']);
					...
					if ($s_role_options)
					{
						$s_role_options = '<option value="0"' . (!$current_role_id ? ' selected="selected"' : '') . ' title="' . htmlspecialchars($this->language->lang('NO_ROLE_ASSIGNED_EXPLAIN')) . '">' . $this->language->lang('NO_ROLE_ASSIGNED') . '</option>' . $s_role_options;
					}
		...
		else
		{
			foreach ($ug_names_ary as $ug_id => $ug_name)
			{
				...
				$this->template->assign_block_vars($tpl_mask_p, [
					...
					'USER_GROUPS_DEFAULT'	=> $user_mode === 'user' && isset($user_groups_default[$ug_id]) && !empty($user_groups_default[$ug_id]) ? implode($this->language->lang('COMMA_SEPARATOR'), $user_groups_default[$ug_id]) : '',
					'USER_GROUPS_CUSTOM'	=> $user_mode === 'user' && isset($user_groups_custom[$ug_id]) && !empty($user_groups_custom[$ug_id]) ? implode($this->language->lang('COMMA_SEPARATOR'), $user_groups_custom[$ug_id]) : '',
					'L_ACL_TYPE'			=> $l_acl_type,
	...
	public function display_role_mask(array &$hold_ary)
	{
		...
		foreach ($forum_names as $forum_id => $forum_name)
		{
			...
			$this->template->assign_block_vars('role_mask', [
				'NAME'				=> $forum_id == 0 ? $this->language->lang('GLOBAL_MASK') : $forum_name,
				'FORUM_ID'			=> $forum_id,
			]);
			...
			if (!empty($auth_ary['groups']))
			{
				...
				while ($row = $this->db->sql_fetchrow($result))
				{
					$this->template->assign_block_vars('role_mask.groups', [
						...
						'U_PROFILE'		=> $this->group_helper->get_name_string('profile', $row['group_id'], $row['group_name']),
					]);
				}
				$this->db->sql_freeresult($result);
			}
	...
	public function acl_delete($mode, $ug_id = false, $forum_id = false, $permission_type = false)
	{
		...
		if ($forum_id !== false)
		{
			$where_sql[] = $this->db->sql_in_set('forum_id', array_map('intval', (array) $forum_id));
		}

		if ($ug_id !== false)
		{
			$where_sql[] = $this->db->sql_in_set($id_field, array_map('intval', (array) $ug_id));
		}

		// There seem to be auth options involved,
		...
		if ($permission_type !== false)
		{
			...
			if (!empty($cur_role_auth))
			{
				...
				$sql = 'SELECT ao.auth_option, rd.role_id, rd.auth_setting
					FROM ' . $this->tables['acl_options'] . ' ao,
						' . $this->tables['acl_roles_data'] . ' rd
	...
	public function assign_cat_array(array &$category_array, $tpl_cat, $tpl_mask, $ug_id, $forum_id, $s_view, $show_trace = false)
	{
		...
		foreach ($category_array as $cat => $cat_array)
		{
			 ...
			 *
			$key_array = array_intersect(array_keys($this->language->get_lang_array(), array_map(create_function('$a', 'return "acl_" . $a;'), array_keys($cat_array['permissions'])));
	...
	public function ghost_permissions($from_user_id, $to_user_id)
	{
		...
		$sql = 'UPDATE ' . $this->tables['users'] . "
			SET user_permissions = '" . $this->db->sql_escape($user_permissions) . "',
				user_perm_from = " . (int) $from_user_id . '
