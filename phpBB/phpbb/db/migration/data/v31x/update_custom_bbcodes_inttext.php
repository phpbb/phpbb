...
class update_custom_bbcodes_inttext extends \phpbb\db\migration\migration
{
	...
	public function update_bbcodes_table()
	{
		...
		$sql = 'SELECT bbcode_id, bbcode_match, bbcode_tpl
			FROM ' . BBCODES_TABLE . '
			WHERE bbcode_match ' . $this->db->sql_like_expression($this->db->get_any_char() . 'INTTEXT' . $this->db->get_any_char());
