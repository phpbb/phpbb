...
class post
{
	...
	public function post_details($id, $mode, $action)
	{
			...
			case 'whois':
				if ($this->auth->acl_get('m_info', $post_info['forum_id']))
				{
					...
					$this->template->assign_vars([
						...
						'RETURN_POST'	=> $this->lang->lang('RETURN_POST', '<a href="' . append_sid("{$this->root_path}mcp.$this->php_ext", "i=$id&amp;mode=$mode&amp;p=$post_id") . '">', '</a>'),
						'L_RETURN_POST'	=> $this->lang->lang('RETURN_POST', '', ''),
		...
		$mcp_post_template_data = [
			'U_MCP_ACTION'			=> "$url&amp;i=main&amp;quickmod=1&amp;mode=post_details", // Use this for mode parameters
			...
			'RETURN_TOPIC'			=> $this->lang->lang('RETURN_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", "f={$post_info['forum_id']}&amp;p=$post_id") . "#p$post_id\">", '</a>'),
			'RETURN_FORUM'			=> $this->lang->lang('RETURN_FORUM', '<a href="' . append_sid("{$this->root_path}viewforum.$this->php_ext", "f={$post_info['forum_id']}&amp;start={$start}") . '">', '</a>'),
			...
			'DELETED_IMG'			=> $this->user->img('icon_topic_deleted', $this->lang->lang('POST_DELETED')),
			'EDIT_IMG'				=> $this->user->img('icon_post_edit', $this->lang->lang('EDIT_POST')),
			'SEARCH_IMG'			=> $this->user->img('icon_user_search', $this->lang->lang('SEARCH')),
		...
		if ($this->auth->acl_get('m_report', (int) $post_info['forum_id']))
		{
			...
			if ($row = $this->db->sql_fetchrow($result))
			{
				...
				do
				{
					...
					if (isset($this->lang->get_lang_array()['report_reasons']['TITLE'][strtoupper($row['reason_title'])]) && isset($this->lang->get_lang_array()['report_reasons']['DESCRIPTION'][strtoupper($row['reason_title'])]))
					{
						$row['reason_description'] = $this->lang->get_lang_array()['report_reasons']['DESCRIPTION'][strtoupper($row['reason_title'])];
						$row['reason_title'] = $this->lang->get_lang_array()['report_reasons']['TITLE'][strtoupper($row['reason_title'])];
					}
		...
		if ($this->auth->acl_get('m_info', $post_info['forum_id']))
		{
			...
			if ($rdns_ip_num !== 'all')
			{
				$this->template->assign_var('U_LOOKUP_ALL', "$url&amp;i=main&amp;mode=post_details&amp;rdns=all");
			}
			...
			if (!empty($users_ary))
			{
				...
				foreach ($users_ary as $user_id => $user_row)
				{
					$this->template->assign_block_vars('userrow', [
						...
						'L_POST_S'		=> $user_row['postings'] == 1 ? $this->lang->lang('POST') : $this->lang->lang('POSTS'),
			...
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				$this->template->assign_block_vars('iprow', [
					...
					'L_POST_S'		=> $row['postings'] == 1 ? $this->lang->lang('POST') : $this->lang->lang('POSTS'),
