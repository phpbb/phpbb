...
class front
{
	...
	function mcp_front_view($id, $mode, $action)
	{
		...
		if ($module->loaded('queue'))
		{
			...
			$this->template->assign_var('S_SHOW_UNAPPROVED', (!empty($forum_list)) ? true : false);
			...
			if (!empty($forum_list))
			{
				...
				if ($total)
				{
					...
					while ($row = $this->db->sql_fetchrow($result))
					{
						$this->template->assign_block_vars('unapproved', [
							...
							'SUBJECT'		=> ($row['post_subject']) ? $row['post_subject'] : $this->language->lang('NO_SUBJECT'),
				...
				$this->template->assign_vars([
					...
					'L_UNAPPROVED_TOTAL'	=> $this->user->lang('UNAPPROVED_POSTS_TOTAL', (int) $total),
		...
		if ($module->loaded('reports'))
		{
			...
			$this->template->assign_var('S_SHOW_REPORTS', (!empty($forum_list)) ? true : false);
			...
			if (!empty($forum_list))
			{
				...
				$this->template->assign_vars([
					'L_REPORTS_TOTAL'	=> $this->user->lang('REPORTS_TOTAL', (int) $total),
		...
		if ($module->loaded('pm_reports') && $this->auth->acl_get('m_pm_report'))
		{
			...
			$this->template->assign_vars([
				'L_PM_REPORTS_TOTAL'	=> $this->user->lang('PM_REPORTS_TOTAL', (int) $total),
