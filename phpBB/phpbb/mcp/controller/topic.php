...
class topic
{
	...
	public function main($page)
	{
		...
		if (($action === 'restore' || $action === 'approve') && $this->auth->acl_get('m_approve', $topic_info['forum_id']))
		{
			...
			{
				$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_view_topic', $params) . '">&laquo; ', '</a>');
		...
		$sql_where .= 'p.topic_id = ' . (int) $topic_id . '
		...
		foreach ($rowset as $current_row_number => $row)
		{
			...
			$post_row = [
				...
				'RETURN_TOPIC'			=> $this->language->lang('RETURN_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", 't=' . $topic_id) . '">', '</a>'),
		...
		$this->template->assign_vars([
			...
			'TO_TOPIC_ID'		=> $to_topic_id,
			'TO_TOPIC_INFO'		=> $to_topic_id ? $this->language->lang('YOU_SELECTED_TOPIC', $to_topic_id, '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $to_topic_info['forum_id'] . '&amp;t=' . $to_topic_id) . '">' . $to_topic_info['topic_title'] . '</a>') : '',
			...
			'RETURN_TOPIC'		=> $this->language->lang('RETURN_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", "f={$topic_info['forum_id']}&amp;t={$topic_info['topic_id']}&amp;start=$start") . '">', '</a>'),
			'RETURN_FORUM'		=> $this->language->lang('RETURN_FORUM', '<a href="' . append_sid("{$this->root_path}viewforum.$this->php_ext", "f={$topic_info['forum_id']}&amp;start=$start") . '">', '</a>'),

			'TOTAL_POSTS'		=> $this->language->lang('VIEW_TOPIC_POSTS', (int) $total),
		]);

		return $this->helper->render('mcp_topic.html', $this->language->lang('MCP_MAIN_TOPIC_VIEW'));
	}
	...
	protected function split_topic($action, $topic_id, $to_forum_id, $subject)
	{
		...
		if (confirm_box(true))
		{
			...
			$return_link = $this->language->lang('RETURN_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']) . '">', '</a>') . '<br /><br />' . $this->language->lang('RETURN_NEW_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
			...
			return $this->helper->message($this->language->lang('TOPIC_SPLIT_SUCCESS') . '<br /><br />' . $return_link);
		}
		else
		{
	...
	protected function merge_posts($topic_id, $to_topic_id)
	{
		...
		if (confirm_box(true))
		{
			...
			if ($row)
			{
				$return_link .= $this->language->lang('RETURN_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $row['forum_id'] . '&amp;t=' . $topic_id) . '">', '</a>');
			}
			else
			{
			...
			$return_link .= ($return_link ? '<br /><br />' : '') . $this->language->lang('RETURN_NEW_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
			$redirect = $this->request->variable('redirect', "{$this->root_path}viewtopic.$this->php_ext?f=$to_forum_id&amp;t=$to_topic_id");
			$redirect = reapply_sid($redirect);
			...
			return $this->helper->message($this->language->lang('POSTS_MERGED_SUCCESS') . '<br /><br />' . $return_link);
		}
		else
		{
