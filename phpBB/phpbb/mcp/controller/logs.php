...
class logs
{
	...
	public function main($mode, $page = 1)
	{
		...
		$delete_marked	= $action === 'del_marked';
		...
		// Delete entries if requested and authorised
		if (($delete_marked || $delete_all) && $this->auth->acl_get('a_clearlogs'))
		{
			if (confirm_box(true))
			{
		...
		return $this->helper->render('mcp_logs.html', $this->language->lang('MCP_LOGS'));
	}
}
