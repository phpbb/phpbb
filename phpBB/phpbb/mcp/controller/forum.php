...
class forum
{
	...
	function mcp_forum_view($id, $mode, $action, $forum_info)
	{
		...
		$this->template->assign_vars([
			...
			'U_VIEW_FORUM_LOGS'		=> ($this->auth->acl_gets('a_', 'm_', $forum_id) && $module->loaded('logs')) ? append_sid("{$this->root_path}mcp.$this->php_ext", 'i=logs&amp;mode=forum_logs&amp;f=' . $forum_id) : '',

			'S_MCP_ACTION'			=> $url . "&amp;i=$id&amp;forum_action=$action&amp;mode=$mode&amp;start=$start" . (($merge_select) ? $selected_ids : ''),

			'TOTAL_TOPICS'			=> $this->user->lang('VIEW_FORUM_TOPICS', (int) $forum_topics),
		]);
		...
		{
			$read_tracking_join = ' LEFT JOIN ' . TOPICS_TRACK_TABLE . ' tt ON (tt.topic_id = t.topic_id AND tt.user_id = ' . $this->user->data['user_id'] . ')';
	...
	function mcp_resync_topics($topic_ids)
	{
		...
		$msg = (count($topic_ids) == 1) ? $this->language->lang('TOPIC_RESYNC_SUCCESS') : $this->language->lang('TOPICS_RESYNC_SUCCESS');
	...
	function merge_topics($forum_id, $topic_ids, $to_topic_id)
	{
		if (!count($topic_ids))
		{
			$this->template->assign_var('MESSAGE', $this->language->lang('NO_TOPIC_SELECTED'));
			return;
		}
		if (!$to_topic_id)
		{
