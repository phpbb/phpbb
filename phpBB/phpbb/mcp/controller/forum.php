...
class forum
{
	...
	public function main($page)
	{
		...
		if (empty($forum_info))
		{
			return trigger_error('NO_FORUM', E_USER_WARNING);
		}
		...
		switch ($action)
		{
			...
			case 'merge_topic':
				if ($to_topic_id)
				{
					...
					{
						$this->template->assign_var('MESSAGE', $this->language->lang($e->getMessage()));
					}
				}
			break;
		}
		...
		$this->template->assign_vars([
			...
			'TOTAL_TOPICS'			=> $this->language->lang('VIEW_FORUM_TOPICS', (int) $forum_topics),
		]);
		...
		foreach ($topic_list as $topic_id)
		{
			...
			$topic_row = [
				'ATTACH_ICON_IMG'		=> ($this->auth->acl_get('u_download') && $this->auth->acl_get('f_download', $row_ary['forum_id']) && $row_ary['topic_attachment']) ? $this->user->img('icon_topic_attach', $this->language->lang('TOTAL_ATTACHMENTS')) : '',
		...
		return $this->helper->render('mcp_forum.html', $this->language->lang('MCP_MAIN_FORUM_VIEW'));
	}
	...
	public function resync_topics(array $topic_ids, $mode)
	{
		...
		if (empty($topic_ids))
		{
			return trigger_error($this->language->lang('NO_TOPIC_SELECTED') . $return, E_USER_WARNING);
		}
		...
		$return		= $this->language->lang('RETURN_PAGE', '<a href="' . $u_back . '">', '</a>');
		$message	= count($topic_ids) === 1 ? $this->language->lang('TOPIC_RESYNC_SUCCESS') : $this->language->lang('TOPICS_RESYNC_SUCCESS');
	...
	protected function merge_topics($forum_id, array $topic_ids, $to_topic_id)
	{
		...
		if (confirm_box(true))
		{
			...
			$l_back		= $this->language->lang('RETURN_PAGE', '<a href="' . $u_back . '">', '</a>');
			$l_topic	= $this->language->lang('RETURN_NEW_TOPIC', '<a href="' . $u_topic . '">', '</a>');

			$message	= $this->language->lang('POSTS_MERGED_SUCCESS');
