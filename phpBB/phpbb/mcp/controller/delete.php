...
class delete
{
	...
	public function delete_posts(array $post_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_post')
	{
		...
		if (confirm_box(true) && $is_soft)
		{
			...
			{
				$return_link[] = $this->language->lang('RETURN_TOPIC', '<a href="' . append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id") . '">', '</a>');
			}

			$return_link[] = $this->language->lang('RETURN_FORUM', '<a href="' . append_sid("{$this->root_path}viewforum.$this->php_ext", 'f=' . $forum_id) . '">', '</a>');
		}
		...
		else
		{
			...
			$this->template->assign_vars([
				'DELETE_POST_PERMANENTLY_EXPLAIN'	=> $this->language->lang('DELETE_POST_PERMANENTLY', (int) count($post_ids)),
		...
		else
		{
			...
			return $this->helper->message($success_msg . '<br /><br />' . $this->language->lang('RETURN_PAGE', '<a href="' . $redirect . '">', '</a>') . '<br /><br />' . implode('<br /><br />', $return_link));
		}
	}
	...
	public function delete_topics(array $topic_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_topic')
	{
		...
		else
		{
			...
			$this->template->assign_vars([
				'DELETE_TOPIC_PERMANENTLY_EXPLAIN'	=> $this->language->lang('DELETE_TOPIC_PERMANENTLY', count($topic_ids)),
				'S_ALLOWED_DELETE'					=> (bool) $this->auth->acl_get('m_delete', $forum_id),
				'S_ALLOWED_SOFTDELETE'				=> (bool) $this->auth->acl_get('m_softdelete', $forum_id),
		...
		{
			$return_page = $this->language->lang('RETURN_' . $redirect_message, '<a href="' . $redirect . '">', '</a>');

			$this->helper->assign_meta_refresh_var(3, $redirect);

			return $this->helper->message($this->language->lang($success_msg) . '<br /><br />' . $return_page);
		}
	}
}
