...
class warn
{
	...
	public function warn_overview()
	{
		...
		return $this->helper->render('mcp_warn_front.html', $this->language->lang('MCP_WARN'));
	}
	...
	public function warn_list($page = 1)
	{
		...
		$this->template->assign_vars([
			'TOTAL_USERS'			=> $this->language->lang('LIST_USERS', (int) $user_count),
		...
		return $this->helper->render('mcp_warn_list.html', $this->language->lang('MCP_WARN_LIST'));
	}
	...
	public function warn_user($u = 0)
	{
		...
		$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_warn_user') . '">&laquo; ', '</a>');
		...
		if ($user_id || $this->request->is_set_post('submituser'))
		{
			...
			{
				return trigger_error($this->language->lang('CANNOT_WARN_SELF') . $return, E_USER_WARNING);
			}

			$user_id = $user_row['user_id'];

			// Include the user identifier in the return link
			$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_warn_user', ['u' => $user_id]) . '">&laquo; ', '</a>');
			...
			if ($warning && $action === 'add_warning')
			{
				...
				if ($s_mcp_warn_user)
				{
					...
					$return = $this->language->lang('RETURN_PAGE', '<a href="' . $u_notes . '">', '</a>');
				...
				return trigger_error($message . $return, E_USER_WARNING);
			}
		...
		return $this->helper->render('mcp_warn_user.html', $this->language->lang('MCP_WARN_USER'));
	}
	...
	public function warn_post()
	{
		...
		$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_warn_overview') . '">&laquo; ', '</a>');
		...
		if ($user_row === false)
		{
			return trigger_error($this->language->lang('NO_POST') . $return, E_USER_WARNING);
		}

		// There is no point issuing a warning to ignored users (ie anonymous and bots)
		if ($user_row['user_type'] == USER_IGNORE)
		{
			return trigger_error($this->language->lang('CANNOT_WARN_ANONYMOUS') . $return, E_USER_WARNING);
		}
		...
		if ($row !== false)
		{
			return trigger_error($this->language->lang('ALREADY_WARNED') . $return, E_USER_WARNING);
		}
		...
		// Set up the new return link and include the identifiers
		$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_warn_post', ['f' => $forum_id, 't' => $topic_id, 'p' => $post_id]) . '">&laquo; ', '</a>');
		...
		$this->template->assign_vars([
			...
			'L_WARNING_POST_DEFAULT'	=> $this->language->lang('WARNING_POST_DEFAULT', generate_board_url() . "/viewtopic.$this->php_ext?f=$forum_id&amp;p=$post_id#p$post_id"),
		...
		return $this->helper->render('mcp_warn_post.html', $this->language->lang('MCP_WARN_POST'));
	}
	...
	protected function add_warning(array $user_row, $warning, $send_pm = true, $post_id = 0)
	{
		if ($send_pm)
		{
			...
			if ($user_row['user_lang'] != $this->language->get_used_language())
			{
				$lang = [];
			...
			else
			{
				...
				$warn_pm_body = $this->language->lang('WARNING_PM_BODY', $warning);
			}
