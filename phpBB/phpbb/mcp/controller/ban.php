...
class ban
{
	...
	public function main($id, $mode)
	{
		...
		{
			include($this->root_path . 'includes/acp/acp_ban.' . $this->php_ext);
		}
		...
		if ($bansubmit)
		{
			...
			if ($ban)
			{
				if (confirm_box(true))
				{
					...
					trigger_error($this->language->lang('BAN_UPDATE_SUCCESSFUL') . '<br /><br /><a href="' . $this->u_action . '">&laquo; ' . $this->language->lang('BACK_TO_PREV') . '</a>');
				}
				else
				{
					$hidden_fields = [
		...
		$this->page_title = $this->language->lang(strtoupper($mode) . '_BAN');

		$l_ban_explain = $this->language->lang(strtoupper($mode) . '_BAN_EXPLAIN');
