...
class notes
{
	...
	public function main($u = 0, $page = 1)
	{
		...
		if ($user_id || $this->request->is_set_post('submituser'))
		{
			...
			if ($user_row === false)
			{
				$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_notes_user') . '">&laquo; ', '</a>');
			...
			if (($del_marked || $del_all) && $this->auth->acl_get('a_clearlogs'))
			{
				...
				if ($where_sql || $del_all)
				{
					...
					{
						$return = '<br><br>' . $this->language->lang('RETURN_PAGE', '<a href="' . $this->helper->route('mcp_notes_user', ['u' => $user_id]) . '">&laquo; ', '</a>');
					...
					else
					{
						...
						$return = $this->language->lang('RETURN_PAGE', '<a href="' . $u_mode . '">', '</a>');

						$this->helper->assign_meta_refresh_var(3, $u_mode);

						return $this->helper->message($this->language->lang($message) . '<br /><br />' . $return);
					}
				}
			}
			...
			if ($user_note && $action === 'add_feedback')
			{
				...
				return $this->helper->message($this->language->lang('USER_FEEDBACK_ADDED') . '<br /><br />' . $return);
			}
			...
			$this->template->assign_vars([
				...
				'TOTAL_REPORTS'		=> $this->language->lang('LIST_REPORTS', (int) $log_count),
			...
			return $this->helper->render('mcp_notes_user.html', $this->language->lang('MCP_NOTES'));
		}
		...
		return $this->helper->render('mcp_notes_front.html', $this->language->lang('MCP_NOTES'));
	}
}
