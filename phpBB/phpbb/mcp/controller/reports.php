...
class reports
{
		...
		switch ($mode)
		{
			case 'details':
				...
				foreach (array_keys($reason) as $type)
				{
					...
					$reason[$type] = $this->lang->is_set($type_array) ? $this->lang->lang($type_array) : $reason[$type];
				}
				...
				return $this->helper->render('mcp_post.html', $this->lang->lang('MCP_REPORT_DETAILS'));
			break;
			...
			case 'closed':
				...
				if ($topic_id)
				{
					...
					if (empty($topic_info))
					{
						throw new back_exception(404, 'TOPIC_NOT_EXIST', $route);
					}
			...
			default:
				throw new back_exception(400, 'NO_MODE', 'mcp_reports_open');
			break;
		}
	}
	...
	public function close_report(array $report_id_list, $mode, $action, $pm = false)
	{
		...
		if ($pm)
		{
			...
			{
				throw new back_exception(403, 'NOT_AUTHORISED', $route);
			}
		}
		else
		{
		...
		if (confirm_box(true))
		{
			...
			$return_forum = '';
			$return_topic = '';
			$return_page = $this->lang->lang('RETURN_PAGE', '<a href="' . $redirect . '">"', '</a>');
			...
			return $this->helper->message($this->lang->lang($message) . '<br /><br />' . $return_forum . $return_topic . $return_page);
		}
		else
		{
