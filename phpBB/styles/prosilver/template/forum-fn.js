...
(function ($) {
	...
	*/
	function marklist(id, name, state) {
		...
		$('#' + id + ' input[type=checkbox][name]').each(() => {
			const $this = $(this);
	...
	*/
	function activateSubPanel(p, panels) {
		...
		if (typeof panels === 'undefined') {
			panels = $('.sub-panels a[data-subpanel]').map(() => {
				return this.getAttribute('data-subpanel');
			});
		}

		for (i = 0; i < panels.length; i++) {
			$('#' + panels[i]).css('display', panels[i] === showPanel ? 'block' : 'none');
			$('#' + panels[i] + '-tab').toggleClass('activetab', panels[i] === showPanel);
		}
	}
	...
	*/
	$('form input[type=text], form input[type=password]').on('keypress', e => {
	...
	*/
	function parseDocument($container) {
		...
		*/
		$container.find('input[data-reset-on-edit]').on('keyup', () => {
		...
		* Pagination
		*/
		$container.find('.pagination .page-jump-form :button').click(() => {
			const $input = $(this).siblings('input.inputbox');
			pageJump($input);
		});

		$container.find('.pagination .page-jump-form input.inputbox').on('keypress', event => {
		...
		$container.find('.pagination .dropdown-trigger').click(() => {
		...
		*/
		$container.find('.breadcrumbs:not([data-skip-responsive])').each(() => {
			const $this = $(this);
		...
		*/
		$container.find('.topiclist.responsive-show-all > li > dl').each(() => {
			const $this = $(this);
		...
		*/
		$container.find('.topiclist.responsive-show-columns').each(() => {
			const $list = $(this);
			const headers = [];
			let headersLength = 0;

			// Find all headers, get contents
			$list.prev('.topiclist').find('li.header dd').not('.mark').each(() => {
		...
		* Responsive tables
		*/
		$container.find('table.table1').not('.not-responsive').each(() => {
			const $this = $(this);
		...
		*/
		$container.find('table.responsive > tbody').not('.responsive-skip-empty').each(() => {
		...
		* Responsive tabs
		*/
		$container.find('#tabs, #minitabs').not('[data-skip-responsive]').each(() => {
			const $this = $(this);
		 ...
		 */
		$container.find('#navigation').each(() => {
		...
		*/
		$container.find('[data-responsive-text]').each(() => {
			const $this = $(this);
